...
class ImportOnDeviceTest {
    ...
    @Test fun parseOnDevice() {
        ...
        android.util.Log.i("IMPORT_TEST", "format=${r.format} workouts=${r.workoutCount} sets=${r.setCount} cardio=${r.cardioRows} skipped=${r.skippedRows}")
    ...
    @Test fun parseThenInsertOnDevice() = runBlocking {
        ...
        android.util.Log.i("IMPORT_TEST", "parsed=${r.workoutCount} inserted=$inserted readBack=$count")
        db.close()
